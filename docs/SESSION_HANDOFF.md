...
**Обновлено:** 2026-07-12 (сессия 1 — ЗАКРЫТА; итоговый статус ниже)

## TL;DR для следующей сессии

Сайт **https://казанская.вмалмыже.рф/** живой: M1 закрыт, M2 почти готов (дизайн/фото/логотип/фон-лента на проде, PR #1–#16). Скилы `/start` `/close_session` `/obriv` заведены, экосистемная карта — в CLAUDE.md. **Следующий шаг:** афиша-2026 → Events в админке (мониторить пакет района) + темы 2017–2025 через SARAFAN + Метрика.
...
## ✅ Логотип + фото владельца DONE (PR #11–#13)

- **SARAFAN-ключ работает** (владелец добавил в env setka); ВК-харвест выполнен (альбом РЦКД «Ярмарка Казанская 2024 Карнавал»), но затем **заменён фотоархивом владельца** (Я.Диск-папка https://disk.yandex.ru/d/Evq5njodhTjunw, ~156 фото Казанской-2024) — качество выше, права чище. Реестр `lib/galleryPhotos.ts` («Фотоархив оргкомитета»), vkCredits удалён.
- **Официальный логотип** (Я.Диск от владельца, чистый PNG 600×732 c альфой): `public/decor/logo.png` (шапка/подвал/hero главной) + favicon `src/app/icon.png`. Оригиналы логотипа: https://disk.yandex.ru/i/XLSnDEL-TZv9mw (исходник в галерее не публикуем).
- Скрипты-помощники в scratchpad: `sarafan.mjs` (ВК через шлюз, см. memory vk-via-sarafan), `yadisk.mjs` (публичные папки Я.Диска через cloud-api).
