...
|---|---|---|
| **Секреты проекта** (восстановление/зеркало, ADR-0006) | **KARMAN**, комната `kazanskayamalmyzh` | Зеркалит **шаг деплоя паспортом** (ADR-0012, D-061): предъявляет OIDC-удостоверение прогона `audience=karman-vault`, получает сессионный токен комнаты на 60 мин, пишет, самоотзывается. Постоянного токена в маршруте нет — на dev-машине держать ключи комнаты не нужно. Адрес API — `secrets.SECRETS_VAULT_URL` (в файл не пишем, D-038). Прод читает из env-файла на боксе (#008) — KARMAN только зеркало. **Новый секрет → в `secrets` репозитория и в список зеркалируемого шага.** |
