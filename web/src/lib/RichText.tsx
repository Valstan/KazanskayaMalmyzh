...
// Минимальный серверный рендер Lexical-richText → React (харвест вМалмыже).
// Абзацы, h2/h3, жирный/курсив/подчёркивание/зачёркивание, ссылки, списки,
// hr, переносы, картинки в тексте (узел `upload`, mediaMap — id → url) и видео
// ВК (ссылка на vk.com/video… → iframe video_ext.php). Сложные узлы — пусто.
...
// URL картинки из upload-узла. Payload 3: relationTo/value на уровне узла
// (version 3); version 2 держал их внутри fields. При population value —
// объект медиа (id/url), в сырой БД — id.
export function resolveUploadUrl(node: LexNode, mediaMap?: Record<string, string | undefined>): string | null {
  let rel: unknown = node.relationTo
  let value: unknown = node.value
  const legacyFields = node.fields as { relationTo?: string; value?: string | number | null } | undefined
...
export function RichText({ data, mediaMap }: { data: unknown; mediaMap?: Record<string, string | undefined> }) {
    ...
    <>
      {root.children.map((node, i) => {
        ...
        if (type === 'list') {
          ...
          const lis = items.map((li, j) => <li key={`${key}-${j}`}>{renderInline(li.children, `${key}-${j}`)}</li>)
          return ordered ? <ol key={key}>{lis}</ol> : <ul key={key}>{lis}</ul>
        }
        if (type === 'horizontalrule') return <hr key={key} />
        if (type === 'paragraph') return <p key={key}>{renderInline(node.children, key)}</p>
        if (type === 'upload') {
          const url = resolveUploadUrl(node, mediaMap)
          if (!url) return null
          return <img key={key} className="post-inline-img" src={url} alt="" loading="lazy" />
        }
        return null
      })}
    </>
