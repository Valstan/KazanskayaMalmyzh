...
export default buildConfig({
  ...
  //
  // Ключ задаётся спредом, а не как `email: undefined`. На поведение это не
  // влияет — Payload в обоих случаях подставляет свою консольную заглушку
  // (`index.js`: ветка `else` → `consoleEmailAdapter`), — но отсутствие ключа
  // честнее выражает «почта не настроена», чем ключ со значением undefined.
